...
class AgentWrapper:
    def __init__(self, model_path = "hex_dqn_agent_2024-06-25_11-43-39.pth", size = 7):
        ...
        self.model.load_state_dict(torch.load(model_white_path, map_location=torch.device('cpu')))
        self.model.eval()
        self.size = size

    def __call__(self, board, action_set):
        state = hex_engine.get_state_tensor(board)
    ...
    def __call__(self, board, action_set):
        ...
        state = hex_engine.get_state_tensor(recoded_board)
